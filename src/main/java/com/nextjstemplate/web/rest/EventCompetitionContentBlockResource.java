...
import com.nextjstemplate.service.EventCompetitionContentBlockQueryService;
import com.nextjstemplate.service.EventCompetitionContentBlockService;
import com.nextjstemplate.service.criteria.EventCompetitionContentBlockCriteria;
...
public class EventCompetitionContentBlockResource {
    ...
    public EventCompetitionContentBlockResource(EventCompetitionContentBlockService eventCompetitionContentBlockService, EventCompetitionContentBlockRepository eventCompetitionContentBlockRepository, EventCompetitionContentBlockQueryService eventCompetitionContentBlockQueryService) {
    ...
    @PostMapping("")
    public ResponseEntity<EventCompetitionContentBlockDTO> create(@Valid @RequestBody EventCompetitionContentBlockDTO dto) throws URISyntaxException {
        if (dto.getId() != null) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<EventCompetitionContentBlockDTO> update(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody EventCompetitionContentBlockDTO dto) throws URISyntaxException {
        if (dto.getId() == null) {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, dto.getId().toString())).body(eventCompetitionContentBlockService.update(dto));
    }

    @PatchMapping(value = "/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<EventCompetitionContentBlockDTO> partialUpdate(@PathVariable(value = "id", required = false) final Long id, @NotNull @RequestBody EventCompetitionContentBlockDTO dto) throws URISyntaxException {
        if (dto.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, dto.getId())) {
    ...
    @GetMapping("")
    public ResponseEntity<List<EventCompetitionContentBlockDTO>> getAll(EventCompetitionContentBlockCriteria criteria, @org.springdoc.core.annotations.ParameterObject Pageable pageable) {
