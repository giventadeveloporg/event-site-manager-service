...
import com.eventsitemanager.service.EventFeaturedPerformersQueryService;
import com.eventsitemanager.service.EventFeaturedPerformersService;
import com.eventsitemanager.service.criteria.EventFeaturedPerformersCriteria;
 ...
 * {@link com.eventsitemanager.domain.EventFeaturedPerformers}.
 */
@RestController
...
public class EventFeaturedPerformersResource {
    ...
    public ResponseEntity<EventFeaturedPerformersDTO> createEventFeaturedPerformers(
        @Valid @RequestBody EventFeaturedPerformersDTO eventFeaturedPerformersDTO
        ...
        if (eventFeaturedPerformersDTO.getId() != null) {
            throw new BadRequestAlertException("A new eventFeaturedPerformers cannot already have an ID", ENTITY_NAME, "idexists");
        }
        ...
        return ResponseEntity
            ...
            .headers(
                HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, eventFeaturedPerformersDTO.getId().toString())
            )
    ...
    public ResponseEntity<EventFeaturedPerformersDTO> updateEventFeaturedPerformers(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, eventFeaturedPerformersDTO.getId().toString()))
            .body(eventFeaturedPerformersDTO);
    }
    ...
    public ResponseEntity<EventFeaturedPerformersDTO> partialUpdateEventFeaturedPerformers(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody EventFeaturedPerformersDTO eventFeaturedPerformersDTO
    ) throws URISyntaxException {
        log.debug("REST request to partial update EventFeaturedPerformers partially : {}, {}", id, eventFeaturedPerformersDTO);
        if (eventFeaturedPerformersDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, eventFeaturedPerformersDTO.getId())) {
        ...
        Optional<EventFeaturedPerformersDTO> result = eventFeaturedPerformersService.partialUpdate(eventFeaturedPerformersDTO);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, eventFeaturedPerformersDTO.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<EventFeaturedPerformersDTO>> getAllEventFeaturedPerformers(
        ...
        final Page<EventFeaturedPerformersDTO> page = eventFeaturedPerformersQueryService.findByCriteria(criteria, pageable);
    ...
    public ResponseEntity<List<EventFeaturedPerformersDTO>> getEventFeaturedPerformersByEventId(@PathVariable("eventId") Long eventId) {
    ...
    public ResponseEntity<List<EventFeaturedPerformersDTO>> getEventFeaturedPerformersByEventIdAndActive(
        ...
        List<EventFeaturedPerformersDTO> eventFeaturedPerformers = eventFeaturedPerformersService.findByEventIdAndIsActive(
            eventId,
            isActive
        );
        return ResponseEntity.ok().body(eventFeaturedPerformers);
    ...
    public ResponseEntity<List<EventFeaturedPerformersDTO>> getEventFeaturedPerformersByEventIdOrderByPriority(
        ...
        List<EventFeaturedPerformersDTO> eventFeaturedPerformers = eventFeaturedPerformersService.findByEventIdOrderByPriorityRanking(
            eventId
        );
        return ResponseEntity.ok().body(eventFeaturedPerformers);
    }
    ...
    public ResponseEntity<List<EventFeaturedPerformersDTO>> getEventFeaturedPerformersByEventIdAndHeadliner(
        ...
        log.debug("REST request to get headliner EventFeaturedPerformers for event : {} and headliner status : {}", eventId, isHeadliner);
        List<EventFeaturedPerformersDTO> eventFeaturedPerformers = eventFeaturedPerformersService.findByEventIdAndIsHeadliner(
            eventId,
            isHeadliner
        );
        return ResponseEntity.ok().body(eventFeaturedPerformers);
    ...
    /**
     * {@code GET  /event-featured-performers/tenant-level} : get all tenant-level eventFeaturedPerformers
     * (not associated with any event).
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of tenant-level eventFeaturedPerformers in body.
     */
    @GetMapping("/tenant-level")
    public ResponseEntity<List<EventFeaturedPerformersDTO>> getTenantLevelEventFeaturedPerformers() {
        String tenantId = TenantContext.getCurrentTenant();
        log.debug("REST request to get tenant-level EventFeaturedPerformers for tenant : {}", tenantId);
        List<EventFeaturedPerformersDTO> eventFeaturedPerformers = eventFeaturedPerformersService.findAllTenantLevelFeaturedPerformers(
            tenantId
        );
        return ResponseEntity.ok().body(eventFeaturedPerformers);
    }
    ...
    /**
     * {@code GET  /event-featured-performers/available-for-event} : get all eventFeaturedPerformers available
     * for association with a specific event.
     *
     * @param eventId the id of the event.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of available eventFeaturedPerformers in body.
     */
    @GetMapping("/available-for-event")
    public ResponseEntity<List<EventFeaturedPerformersDTO>> getAvailableEventFeaturedPerformersForEvent(
        @RequestParam("eventId") Long eventId
    ) {
        String tenantId = TenantContext.getCurrentTenant();
        log.debug("REST request to get available EventFeaturedPerformers for tenant : {} and event : {}", tenantId, eventId);
        List<EventFeaturedPerformersDTO> eventFeaturedPerformers = eventFeaturedPerformersService.findAvailableFeaturedPerformersForEvent(
            tenantId,
            eventId
        );
        return ResponseEntity.ok().body(eventFeaturedPerformers);
    ...
    /**
     * {@code PATCH  /event-featured-performers/:id/disassociate} : Disassociate an eventFeaturedPerformer
     * from its event.
     *
     * @param id the id of the eventFeaturedPerformersDTO to disassociate.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated eventFeaturedPerformersDTO.
     */
    @PatchMapping("/{id}/disassociate")
    public ResponseEntity<EventFeaturedPerformersDTO> disassociateEventFeaturedPerformerFromEvent(@PathVariable("id") Long id) {
        log.debug("REST request to disassociate EventFeaturedPerformers : {} from its event", id);
        EventFeaturedPerformersDTO result = eventFeaturedPerformersService.disassociateFromEvent(id);
        return ResponseEntity
            .ok()
    ...
    /**
     * {@code PATCH  /event-featured-performers/:id/associate/:eventId} : Associate an eventFeaturedPerformer
     * with a specific event.
     *
     * @param id      the id of the eventFeaturedPerformersDTO to associate.
     * @param eventId the id of the event to associate with.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated eventFeaturedPerformersDTO.
     */
    @PatchMapping("/{id}/associate/{eventId}")
    public ResponseEntity<EventFeaturedPerformersDTO> associateEventFeaturedPerformerWithEvent(
        @PathVariable("id") Long id,
        @PathVariable("eventId") Long eventId
    ) {
        log.debug("REST request to associate EventFeaturedPerformers : {} with event : {}", id, eventId);
        EventFeaturedPerformersDTO result = eventFeaturedPerformersService.associateWithEvent(id, eventId);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, id.toString()))
            .body(result);
