...
import com.eventsitemanager.repository.EventContactsRepository;
import com.eventsitemanager.security.TenantContext;
import com.eventsitemanager.service.EventContactsQueryService;
import com.eventsitemanager.service.EventContactsService;
import com.eventsitemanager.service.criteria.EventContactsCriteria;
...
/**
 * REST controller for managing {@link com.eventsitemanager.domain.EventContacts}.
 */
@RestController
...
public class EventContactsResource {
    ...
    public ResponseEntity<EventContactsDTO> createEventContacts(@Valid @RequestBody EventContactsDTO eventContactsDTO)
        ...
        return ResponseEntity
            ...
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, eventContactsDTO.getId().toString()))
    ...
    /**
     * {@code GET  /event-contacts/tenant-level} : get all tenant-level eventContacts
     * (not associated with any event).
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of tenant-level eventContacts in body.
     */
    @GetMapping("/tenant-level")
    public ResponseEntity<List<EventContactsDTO>> getTenantLevelEventContacts() {
        String tenantId = TenantContext.getCurrentTenant();
        log.debug("REST request to get tenant-level EventContacts for tenant : {}", tenantId);
        List<EventContactsDTO> eventContacts = eventContactsService.findAllTenantLevelContacts(tenantId);
        return ResponseEntity.ok().body(eventContacts);
    }

    /**
     * {@code GET  /event-contacts/available-for-event} : get all eventContacts available
     * for association with a specific event.
     *
     * @param eventId the id of the event.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of available eventContacts in body.
     */
    @GetMapping("/available-for-event")
    public ResponseEntity<List<EventContactsDTO>> getAvailableEventContactsForEvent(@RequestParam("eventId") Long eventId) {
        String tenantId = TenantContext.getCurrentTenant();
        log.debug("REST request to get available EventContacts for tenant : {} and event : {}", tenantId, eventId);
        List<EventContactsDTO> eventContacts = eventContactsService.findAvailableContactsForEvent(tenantId, eventId);
        return ResponseEntity.ok().body(eventContacts);
    }

    /**
     * {@code PATCH  /event-contacts/:id/disassociate} : Disassociate an eventContact
     * from its event.
     *
     * @param id the id of the eventContactsDTO to disassociate.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated eventContactsDTO.
     */
    @PatchMapping("/{id}/disassociate")
    public ResponseEntity<EventContactsDTO> disassociateEventContactFromEvent(@PathVariable("id") Long id) {
        log.debug("REST request to disassociate EventContacts : {} from its event", id);
    ...
    /**
     * {@code PATCH  /event-contacts/:id/associate/:eventId} : Associate an eventContact
     * with a specific event.
     *
     * @param id      the id of the eventContactsDTO to associate.
     * @param eventId the id of the event to associate with.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated eventContactsDTO.
     */
    @PatchMapping("/{id}/associate/{eventId}")
    public ResponseEntity<EventContactsDTO> associateEventContactWithEvent(
        @PathVariable("id") Long id,
        @PathVariable("eventId") Long eventId
    ) {
        log.debug("REST request to associate EventContacts : {} with event : {}", id, eventId);
