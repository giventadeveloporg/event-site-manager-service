...
import com.eventsitemanager.service.EventProgramDirectorsQueryService;
import com.eventsitemanager.service.EventProgramDirectorsService;
import com.eventsitemanager.service.criteria.EventProgramDirectorsCriteria;
 ...
 * {@link com.eventsitemanager.domain.EventProgramDirectors}.
 */
@RestController
...
public class EventProgramDirectorsResource {
    ...
    public ResponseEntity<EventProgramDirectorsDTO> createEventProgramDirectors(
        ...
        if (eventProgramDirectorsDTO.getId() != null) {
            throw new BadRequestAlertException("A new eventProgramDirectors cannot already have an ID", ENTITY_NAME, "idexists");
        }
        ...
        return ResponseEntity
            ...
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, eventProgramDirectorsDTO.getId().toString()))
    ...
    public ResponseEntity<EventProgramDirectorsDTO> updateEventProgramDirectors(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, eventProgramDirectorsDTO.getId().toString()))
            .body(eventProgramDirectorsDTO);
    }
    ...
    public ResponseEntity<EventProgramDirectorsDTO> partialUpdateEventProgramDirectors(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody EventProgramDirectorsDTO eventProgramDirectorsDTO
    ) throws URISyntaxException {
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, eventProgramDirectorsDTO.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<EventProgramDirectorsDTO>> getEventProgramDirectorsByEventId(@PathVariable("eventId") Long eventId) {
    ...
    public ResponseEntity<List<EventProgramDirectorsDTO>> getEventProgramDirectorsWithBioByEventId(@PathVariable("eventId") Long eventId) {
    ...
    /**
     * {@code GET  /event-program-directors/tenant-level} : get all tenant-level eventProgramDirectors
     * (not associated with any event).
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of tenant-level eventProgramDirectors in body.
     */
    @GetMapping("/tenant-level")
    public ResponseEntity<List<EventProgramDirectorsDTO>> getTenantLevelEventProgramDirectors() {
        String tenantId = TenantContext.getCurrentTenant();
        log.debug("REST request to get tenant-level EventProgramDirectors for tenant : {}", tenantId);
        List<EventProgramDirectorsDTO> eventProgramDirectors = eventProgramDirectorsService.findAllTenantLevelProgramDirectors(tenantId);
        return ResponseEntity.ok().body(eventProgramDirectors);
    }

    /**
     * {@code GET  /event-program-directors/available-for-event} : get all eventProgramDirectors available
     * for association with a specific event.
     *
     * @param eventId the id of the event.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list
     *         of available eventProgramDirectors in body.
     */
    @GetMapping("/available-for-event")
    public ResponseEntity<List<EventProgramDirectorsDTO>> getAvailableEventProgramDirectorsForEvent(@RequestParam("eventId") Long eventId) {
        String tenantId = TenantContext.getCurrentTenant();
        log.debug("REST request to get available EventProgramDirectors for tenant : {} and event : {}", tenantId, eventId);
        List<EventProgramDirectorsDTO> eventProgramDirectors = eventProgramDirectorsService.findAvailableProgramDirectorsForEvent(
            tenantId,
            eventId
        );
        return ResponseEntity.ok().body(eventProgramDirectors);
    ...
    /**
     * {@code PATCH  /event-program-directors/:id/disassociate} : Disassociate an eventProgramDirector
     * from its event.
     *
     * @param id the id of the eventProgramDirectorsDTO to disassociate.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated eventProgramDirectorsDTO.
     */
    @PatchMapping("/{id}/disassociate")
    public ResponseEntity<EventProgramDirectorsDTO> disassociateEventProgramDirectorFromEvent(@PathVariable("id") Long id) {
        log.debug("REST request to disassociate EventProgramDirectors : {} from its event", id);
        EventProgramDirectorsDTO result = eventProgramDirectorsService.disassociateFromEvent(id);
        return ResponseEntity
            .ok()
    ...
    /**
     * {@code PATCH  /event-program-directors/:id/associate/:eventId} : Associate an eventProgramDirector
     * with a specific event.
     *
     * @param id      the id of the eventProgramDirectorsDTO to associate.
     * @param eventId the id of the event to associate with.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated eventProgramDirectorsDTO.
     */
    @PatchMapping("/{id}/associate/{eventId}")
    public ResponseEntity<EventProgramDirectorsDTO> associateEventProgramDirectorWithEvent(
        @PathVariable("id") Long id,
        @PathVariable("eventId") Long eventId
    ) {
        log.debug("REST request to associate EventProgramDirectors : {} with event : {}", id, eventId);
        EventProgramDirectorsDTO result = eventProgramDirectorsService.associateWithEvent(id, eventId);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, id.toString()))
            .body(result);
