...
import com.eventsitemanager.service.WebhookSignatureValidator;
import com.eventsitemanager.service.WhatsAppLogService;
import com.eventsitemanager.service.dto.MessageStatus;
import com.eventsitemanager.service.dto.WebhookPayloadDTO;
...
public class WhatsAppWebhookResource {
    ...
    public WhatsAppWebhookResource(WebhookSignatureValidator webhookSignatureValidator, WhatsAppLogService whatsAppLogService) {
    ...
    public ResponseEntity<String> handleDeliveryStatus(HttpServletRequest request, @RequestParam(required = false) String tenantId) {
        ...
        try {
            ...
            LOG.info(
                "Successfully processed delivery status webhook for message: {}, tenant: {}",
    ...
    public ResponseEntity<String> handleMessageStatus(HttpServletRequest request, @RequestParam(required = false) String tenantId) {
        ...
        try {
            ...
            LOG.info(
                "Successfully processed message status webhook for message: {}, tenant: {}",
                payload.getMessageId(),
                extractedTenantId
            );
    ...
    private void updateMessageStatus(WebhookPayloadDTO payload) {
        try {
            ...
            LOG.debug("Updated message status for message ID: {} to status: {}", payload.getMessageId(), payload.getStatus());
        } catch (Exception e) {
            LOG.error("Error updating message status for message ID: {}: {}", payload.getMessageId(), e.getMessage(), e);
        }
    }
