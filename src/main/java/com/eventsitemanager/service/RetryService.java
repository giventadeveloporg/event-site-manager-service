...
import com.eventsitemanager.service.exception.WhatsAppRateLimitException;
    ...
    public <T> T executeWithRetry(Supplier<T> supplier, String operationName) throws Exception {
        ...
        for (int attempt = 1; attempt <= retryAttempts; attempt++) {
            try {
                ...
                lastException = e;
                LOG.warn("Rate limit hit for {} (attempt {}/{}): {}", operationName, attempt, retryAttempts, e.getMessage());
                ...
                lastException = e;
                LOG.error("Error executing {} (attempt {}/{}): {}", operationName, attempt, retryAttempts, e.getMessage(), e);
    ...
    public <T> T executeBulkWithRetry(Supplier<T> supplier, String operationName) throws Exception {
        ...
        for (int attempt = 1; attempt <= retryAttempts; attempt++) {
            try {
                ...
                lastException = e;
                LOG.warn("Rate limit hit for bulk {} (attempt {}/{}): {}", operationName, attempt, retryAttempts, e.getMessage());
                ...
                lastException = e;
                LOG.error("Error executing bulk {} (attempt {}/{}): {}", operationName, attempt, retryAttempts, e.getMessage(), e);
