...
import com.eventsitemanager.repository.EventEmailsRepository;
import com.eventsitemanager.service.EventEmailsService;
import com.eventsitemanager.service.dto.EventEmailsDTO;
import com.eventsitemanager.service.mapper.EventEmailsMapper;
...
public class EventEmailsServiceImpl implements EventEmailsService {
    ...
    public EventEmailsServiceImpl(
        EventEmailsRepository eventEmailsRepository,
        EventEmailsMapper eventEmailsMapper,
    ...
    public List<EventEmailsDTO> findByEventId(Long eventId) {
        ...
        return eventEmailsRepository.findByEventId(eventId).stream().map(eventEmailsMapper::toDto).collect(Collectors.toList());
    }
    ...
    public List<EventEmailsDTO> findByEmail(String email) {
        ...
        return eventEmailsRepository.findByEmail(email).stream().map(eventEmailsMapper::toDto).collect(Collectors.toList());
    }
    ...
    public Optional<EventEmailsDTO> findByEventIdAndEmailExact(Long eventId, String email) {
        ...
        return eventEmailsRepository.findOneByEventIdAndEmail(eventId, email).map(eventEmailsMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EventEmailsDTO> findAllTenantLevelEmails(String tenantId) {
        log.debug("Request to get all tenant-level EventEmails for tenant : {}", tenantId);
        return eventEmailsRepository
    ...
    @Override
    @Transactional(readOnly = true)
    public List<EventEmailsDTO> findAvailableEmailsForEvent(String tenantId, Long eventId) {
        log.debug("Request to get available EventEmails for tenant : {} and event : {}", tenantId, eventId);
        return eventEmailsRepository
    ...
    @Override
    public EventEmailsDTO disassociateFromEvent(Long id) {
        log.debug("Request to disassociate EventEmails : {} from its event", id);
        EventEmails eventEmails = eventEmailsRepository
            .findById(id)
            .orElseThrow(() -> new RuntimeException("EventEmails not found with id: " + id));
    ...
    @Override
    public EventEmailsDTO associateWithEvent(Long id, Long eventId) {
        log.debug("Request to associate EventEmails : {} with event : {}", id, eventId);
        EventEmails eventEmails = eventEmailsRepository
            .findById(id)
            .orElseThrow(() -> new RuntimeException("EventEmails not found with id: " + id));
