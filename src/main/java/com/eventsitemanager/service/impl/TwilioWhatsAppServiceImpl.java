...
import com.eventsitemanager.service.RateLimitService;
import com.eventsitemanager.service.RetryService;
...
public class TwilioWhatsAppServiceImpl implements TwilioWhatsAppService {
        ...
        try {
            ...
            return retryService.executeWithRetry(
                () -> {
                        ...
                        try {
                            message =
                                Message
                                    ...
                                    .setMediaUrl(Arrays.asList(new java.net.URI(request.getMediaUrl())))
                                    .create();
                    ...
                    // Create response
                    TwilioWhatsAppResponseDTO response = new TwilioWhatsAppResponseDTO(message.getSid(), message.getStatus().toString());
                    response.setMetadata(
                        Map.of("twilioResponse", message.toString(), "messageType", request.getTemplateId() != null ? "TEMPLATE" : "TEXT")
                    );
            ...
            return new TwilioWhatsAppResponseDTO(null, "FAILED", "SEND_ERROR", e.getMessage());
        }
    }

    @Override
    public TwilioWhatsAppBulkResponseDTO sendBulkMessages(String tenantId, TwilioWhatsAppBulkRequestDTO request) {
        LOG.debug("Sending bulk WhatsApp messages for tenant: {}, recipients: {}", tenantId, request.getRecipients().size());
        ...
        try {
            ...
            return retryService.executeBulkWithRetry(
                () -> {
                    ...
                    for (String recipient : request.getRecipients()) {
                        ...
                        CompletableFuture<TwilioWhatsAppResponseDTO> future = CompletableFuture.supplyAsync(() -> {
                            try {
                    ...
                    List<TwilioWhatsAppResponseDTO> results = futures.stream().map(CompletableFuture::join).collect(Collectors.toList());
                    ...
                    TwilioWhatsAppBulkResponseDTO response = new TwilioWhatsAppBulkResponseDTO(totalSent, totalFailed, results);
                    response.setCampaignId(request.getCampaignId());

                    LOG.info("Completed bulk WhatsApp messages for tenant: {}, sent: {}, failed: {}", tenantId, totalSent, totalFailed);
            ...
            TwilioWhatsAppBulkResponseDTO errorResponse = new TwilioWhatsAppBulkResponseDTO(
                0,
        ...
        try {
            ...
            response.setMetadata(
                Map.of(
                    ...
                    message.toString(),
                    "price",
                    message.getPrice() != null ? message.getPrice() : "N/A",
                    "priceUnit",
                    message.getPriceUnit() != null ? message.getPriceUnit() : "N/A"
                )
            );

            LOG.debug("Retrieved delivery status for tenant: {}, messageId: {}, status: {}", tenantId, messageId, message.getStatus());

            return response;
        } catch (Exception e) {
            LOG.error("Failed to check delivery status for tenant: {}, messageId: {}, error: {}", tenantId, messageId, e.getMessage(), e);

            return new TwilioWhatsAppResponseDTO(messageId, "ERROR", "FETCH_ERROR", e.getMessage());
        }
    }
    ...
    public Map<String, Object> handleWebhook(String tenantId, Map<String, Object> payload) {
        ...
        try {
            ...
            if (messageId != null && status != null) {
                ...
                LOG.info("Updated message status for tenant: {}, messageId: {}, status: {}", tenantId, messageId, status);
            }
    ...
    public void logMessage(WhatsAppLogEntryDTO logEntry) {
        ...
        try {
            ...
            com.eventsitemanager.service.dto.WhatsAppLogDTO whatsAppLogDTO = new com.eventsitemanager.service.dto.WhatsAppLogDTO();
            ...
            LOG.error("Failed to log WhatsApp message for tenant: {}, error: {}", logEntry.getTenantId(), e.getMessage(), e);
