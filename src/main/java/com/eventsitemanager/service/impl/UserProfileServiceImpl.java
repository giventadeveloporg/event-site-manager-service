...
import com.eventsitemanager.domain.enumeration.UserStatusType;
 ...
 * {@link com.eventsitemanager.domain.UserProfile}.
 */
@Service
@Transactional
public class UserProfileServiceImpl implements UserProfileService {
    ...
    @Cacheable(
        ...
        key = "#userId + '_' + (T(com.eventsitemanager.security.TenantContext).getCurrentTenant() != null ? T(com.eventsitemanager.security.TenantContext).getCurrentTenant() : 'no-tenant')",
        unless = "#result == null"
    )
    ...
    public List<UserProfileDTO> findSubscribedUsersByTenantIdWithPagination(String tenantId, int limit, int offset) {
        ...
        return page.getContent().stream().map(userProfileMapper::toDto).collect(Collectors.toList());
    }
    ...
    private void generateEmailSubscriptionTokenIfNeeded(UserProfile userProfile) {
        if (
            ...
            try {
                ...
                log.error("Failed to generate email subscription token for user with email: {}", userProfile.getEmail(), e);
    ...
    private UserProfile refreshEmailSubscriptionTokenIfNeeded(UserProfile userProfile) {
        if (
            ...
            userProfile.getTenantId() != null &&
            !userProfile.getTenantId().trim().isEmpty() &&
            (userProfile.getEmailSubscriptionToken() == null || userProfile.getEmailSubscriptionToken().trim().isEmpty())
        ) {
            try {
                String token = emailSubscriptionTokenService.generateEmailSubscriptionToken(
                    userProfile.getEmail(),
                ...
                log.error("Failed to refresh email subscription token for user with email: {}", userProfile.getEmail(), e);
                // Don't fail the operation if token generation fails
            }
        }
        return userProfile;
