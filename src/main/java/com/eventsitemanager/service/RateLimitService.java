...
public class RateLimitService {
    ...
    public void checkRateLimit(String tenantId, boolean isBulk) {
        ...
        if (rateLimitInfo.getMessageCount() >= limit) {
            LOG.warn("Rate limit exceeded for tenant: {}, isBulk: {}, limit: {}", tenantId, isBulk, limit);
            throw new WhatsAppRateLimitException("Rate limit exceeded. Try again later.", retryDelaySeconds);
        }

        rateLimitInfo.addMessage(now);
        LOG.debug(
            "Rate limit check passed for tenant: {}, isBulk: {}, count: {}/{}",
            tenantId,
            isBulk,
            rateLimitInfo.getMessageCount(),
            limit
        );
    }
